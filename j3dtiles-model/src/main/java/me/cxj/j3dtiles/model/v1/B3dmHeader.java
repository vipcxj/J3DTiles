...
public class B3dmHeader extends BatchHeader {
    ...
    public static final byte magic[] = { 0x62, 0x33, 0x64, 0x6d };
    ...
    public static B3dmHeader read(InputStream is, boolean hasReadMagic, boolean hasReadVersion) throws IOException {
        ...
        if (!hasReadMagic) {
            ...
            if (!Arrays.equals(magic, header.getMagic())) {
                throw new IllegalArgumentException("Invalid input stream. Not a valid b3dm stream. The magic is wrong. Expect b3dm but " + new String(magic) + ".");
            }
        }
        if (!hasReadVersion) {
            int version = dataInputStream.readInt();
            if (version != header.getVersion()) {
                throw new IllegalArgumentException("Invalid input stream. Not a valid b3dm stream. Mismatch version. Expect 1 but " + version + ".");
            }
        }
